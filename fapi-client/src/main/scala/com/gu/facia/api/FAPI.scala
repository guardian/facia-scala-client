...
object FAPI {
  ...
  private def applyDefaultBoost(content: FaciaContent, default: BoostLevel, allowedBoosts: List[BoostLevel]): FaciaContent =
    if (allowedBoosts.contains(content.properties.boostLevel))
      content
    else
      updateContentProperties(content, content.properties.copy(boostLevel = default))
  ...
  def applyDefaultBoostLevels(collectionConfig: CollectionConfig, contents: List[FaciaContent]): List[FaciaContent] = {
    val contentsWithDefaultBoosts = for {
      ...
      boostsConfig <- boostsConfigFor(collectionConfig.collectionType, groupsConfig.config)
    } yield {
      val (result, _) = boostsConfig.foldLeft((List.empty[FaciaContent], contents)) { case ((processed, unprocessed), groupConfig) =>
        val (groupContents, remaining) = unprocessed.splitAt(groupConfig.maxItems)
        (processed ++ groupContents.map(applyDefaultBoost(_, groupConfig.default, groupConfig.allowedBoosts)), remaining)
      }
      result
    }
