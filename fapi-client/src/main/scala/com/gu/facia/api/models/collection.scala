    ...
    val responses: Seq[Response[FaciaContent]] = faciaContent.map {
      case curatedContent: CuratedContent =>
        getMediaAtomData(curatedContent).map {
          case mediaAtomData @ Some(_) => curatedContent.copy(mediaAtomData = mediaAtomData) // [please don't keep this comment] this is a (possibly unnecessary optimisation) so that we only bother to copy the curated content if it actually has a mediaAtm
