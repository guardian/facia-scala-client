...
class CustomSubnavServiceTest extends AnyFreeSpec with Matchers with TestContent {
  ...
  "getSubnavForFront" - {
    "returns matching live subnav for a front id" in {
      ...
      CustomSubnavService.getSubnavForFront(config, "uk", Live) shouldBe Some(liveMatch)
    }
    ...
    "returns matching draft subnav when status is Draft" in {
      ...
      CustomSubnavService.getSubnavForFront(config, "culture", Draft) shouldBe Some(draftMatch)
      CustomSubnavService.getSubnavForFront(config, "culture", Live) shouldBe None
    }
    ...
    "prefers draft match over live in Draft mode, but not in Live mode" in {
      ...
      val config = CustomSubnavConfig(live = List(liveMatch), draft = List(draftMatch))

      CustomSubnavService.getSubnavForFront(config, "sport", Draft) shouldBe Some(draftMatch)
      CustomSubnavService.getSubnavForFront(config, "sport", Live) shouldBe Some(liveMatch)
    }
  }
  ...
  "getSubnavForContent" - {
    "returns matching live subnav for a specific article id" in {
      ...
      val liveMatch = subnav("by-article", List(TargetedPage(Article, "article/123")))
      val config = CustomSubnavConfig(live = List(liveMatch), draft = Nil)

      CustomSubnavService.getSubnavForContent(config, content, Live) shouldBe Some(liveMatch)
    }

    "returns matching draft subnav for one of the article tags" in {
      val content = baseContent.copy(tags = List(Tag("tone/comment", TagType.Keyword, webTitle = "", webUrl = "", apiUrl = "")))
      val draftMatch = subnav("by-tag", List(TargetedPage(HasTag, "tone/comment")))
      val config = CustomSubnavConfig(live = Nil, draft = List(draftMatch))

      CustomSubnavService.getSubnavForContent(config, content, Draft) shouldBe Some(draftMatch)
      CustomSubnavService.getSubnavForContent(config, content, Live) shouldBe None
    }

    "prefers draft match over live in Draft mode, but not in Live mode" in {
      val content = baseContent.copy(id = "article/456")
      val liveMatch = subnav("live-content", List(TargetedPage(Article, "article/456")))
      val draftMatch = subnav("draft-content", List(TargetedPage(Article, "article/456")))
      val config = CustomSubnavConfig(live = List(liveMatch), draft = List(draftMatch))
